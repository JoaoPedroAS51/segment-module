...
export default class Client {
  constructor (ctx, options) {
    ...
    if (typeof this.options.autoPageTracking === 'boolean') {
      this.options.autoPageTracking = {
        enable: this.options.autoPageTracking
      }
    }
  ...
  init () {
    ...
    if (!this.options.debug && this.options.clientKey) {
      loadScript(`https://cdn.segment.com/analytics.js/v1/${this.options.clientKey}/analytics.min.js`, (error, script) => {
        ...
        const poll = setInterval(() => {
          ...
          if (this.options.autoPageTracking.enable === true) {
